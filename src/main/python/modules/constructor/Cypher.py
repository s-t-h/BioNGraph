...
def graph_to_cypher(graph):
    ...
    return CREATE \
           + create_vertex(*[vertex.attributes() for vertex in graph.vs]) \
           + create_edge(*[edge.attributes() for edge in graph.es])
...
def experimental_match(*args):
    ...
    for arg in args:
        ...
        match += ' , '.join(['(v' + strindex + ')',
                             '(vs' + strindex + ')-[e' + strindex + ']->(vt' + strindex + ')'
                             ])

        where += \
            AND.join([
                OR.join(['v' + strindex + '.id' + IS + _wrap(id) for id in arg]),
                OR.join(['vs' + strindex + '.id' + IS + _wrap(id) + OR + 'vt' + strindex + '.id' + IS + _wrap(id) for id in arg])
            ])
